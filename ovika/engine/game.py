...
class Game:
    ...
    def cost_of(self, pl: Player, card: CardDef, x: int = 0,
        ...
        # --- reductions
        red = 0
        if any(q.card.name == "Goblin Electromancer" for q in pl.battlefield) and (card.is_instant or card.is_sorcery):
            red += 1
        if any(q.card.name == "Goblin Warchief" for q in pl.battlefield) and "Goblin" in card.subtypes:
            red += 1
    ...
    def legal_attackers(self, pl: Player) -> List[Permanent]:
        ...
        for perm in pl.battlefield:
            ...
            if perm.tapped:
                continue
            if perm.summon_sick:
                from .terra import anger_haste
                if not anger_haste(self, pl):
                    continue
    ...
    def declare_attackers(self, pl: Player) -> None:
        ...
        # requirements: creatures that "attack each combat if able"
        for perm in self.legal_attackers(pl):
            must = False
            for src in pl.battlefield:
                if src.card.name in ("Goblin Assault", "Goblin Rabblemaster") and src.controller is pl:
                    must = True
            if must and perm not in chosen:
                chosen.append(perm)
        for perm in chosen:
    ...
    def deal_damage(self, source: Permanent, target, amount: int) -> None:
        ...
        if amount <= 0:
            return
        # damage amplifiers (2026 convoke build): City on Fire / Fiery
        # Emancipation triple, Collective Inferno doubles goblin damage,
        # Torbran adds +2 to red sources
        if source is not None:
            ctrl = source.controller
            if any(q.card.name in ("City on Fire", "Fiery Emancipation")
                   for q in ctrl.battlefield):
                amount *= 3
            if any(q.card.name == "Collective Inferno" for q in ctrl.battlefield) and (
                    "Goblin" in source.card.types or "Goblin" in source.card.subtypes):
                amount *= 2
            if any(q.card.name == "Torbran, Thane of Red Fell" for q in ctrl.battlefield) and (
                    "Red" in source.card.colors):
                amount += 2
        if isinstance(target, Player):
            target.life -= amount
            if source.card.is_legendary and not source.token and (
                    source.card is source.owner.commander or
                    (source.card.name == "Esper Terra" and source.owner.commander.name.startswith("Terra,"))):
