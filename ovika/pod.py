"""4-deck pod: Ovika v11 + the three EDHREC decks (Nekusar, Kuja, Minstrel).

Usage:
  python ovika/pod.py --games 12 --seeds 1-12 --turns 14 --workers 4 [--quiet] [--log1]
"""
...
POD = [
    ("ovika deck", "deck_v11.py", "Ovika, Enigma Goliath"),
...
def play_pod_seed(seed: int, turns: int, log_fn=None) -> dict:
    oracle = Oracle()
    decks, commanders = [], []
...
def main():
    ...
    ap.add_argument("--csv", default=os.path.join(HERE, "results_pod.csv"))
    ...
    if workers <= 1:
        ...
        with ProcessPoolExecutor(max_workers=workers) as ex:
            futs = [ex.submit(play_pod_seed, s, args.turns) for s in seeds]
