        ...
        if perm.card.is_land and "sacrifice" in t and "search your library" in t:
            ...
            if m:
                ...
                def _fetch(perm=perm, want=want, enters_tapped=enters_tapped):
                    ...
                    legal = [c for c in pl.library if pred(c)]
                    if pl.commander.name.startswith("Terra,") and legal:
                        have = set()
                        for q in pl.battlefield:
                            if q.card.is_land:
                        ...
                        def _cover(c):
                            ...
                            if "one mana of any color" in (c.text or "") or \
                                    c.name in ("Command Tower", "Exotic Orchard"):
                                cols |= {"R", "G"}
                            return len(cols - have), len(cols)
                        pick = max(legal, key=_cover)
                        pl.library.remove(pick)
                        q = Permanent(card=pick, controller=pl, owner=pl)
                        pl.battlefield.append(q)
                        q.tapped = enters_tapped
                        pl.shuffle()
                        game.log(f"      {pl.name} fetches {pick.name} with {perm.card.name} (Terra fixing)")
                        return
