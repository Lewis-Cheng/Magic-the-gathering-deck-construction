...
def fast_sweep(commander_name: str, n: int = 100_000, turns: int = 10,
               ...
               win_n: int = 30, commander_cmc: int = 4) -> dict:
    """Abstraction-level curve sweep. Not a substitute for engine games, and not a real win rate."""
    if torch is None:
        raise RuntimeError("torch not installed; run without --gpu")
    dev = device()
    ...
    commander_turn = torch.full((n,), turns + 1, device=dev)
    ...
    for t in range(1, turns + 1):
        ...
        mana += rock_draw * 1.3
        just_cast = (~(commander_turn <= t)) & (mana >= commander_cmc)
        commander_turn = torch.where(just_cast, torch.full_like(commander_turn, t), commander_turn)
        active = commander_turn <= t
        ...
        commander_turn = torch.where(removed, torch.full_like(commander_turn, turns + 1), commander_turn)
